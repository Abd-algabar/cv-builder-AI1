...
const TemplateSelector = ({selectedTemplate, onChange}) => {
    ...
    const templates = [
        ...
        {id:'minimal',name:'Minimal',preview:'Minimal design with a single image and clean typography'},
        {id:'minimal-image',name:'Minimal Image',preview:'Ultra-clean design that puts your content front and center'},
    ]
  return (
        ...
        {isOpen && (
            <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                    {templates.map((template)=>(
                        <div key={template.id}  onClick={()=>{onChange(template.id); setIsOpen(false)}} className={`rel p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? 'border-blue-400 bg-blue-100' : 'border-gray-300 hover:bg-gray-100'}`}>
